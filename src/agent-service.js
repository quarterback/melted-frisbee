...
export class AgentService {
  constructor(config) {
    ...
    this.heartbeatIntervalMs = 4 * 60 * 60 * 1000; // 4 hours
  }
  ...
  async initialize() {
    ...
    if (!this.config.apiKey) {
      throw new Error('MOLTBOOK_API_KEY is required. Please register your agent first.');
    }
    ...
    this.behaviors = new AgentBehaviors(this.client, {
      ...
      commentProbability: this.config.commentProbability || 0.3,
    ...
    const checkHeartbeat = async () => {
      ...
      if (!this.lastHeartbeatCheck || (now - this.lastHeartbeatCheck) >= this.heartbeatIntervalMs) {
        ...
        try {
          ...
          console.log('Heartbeat received:', heartbeat.substring(0, 100) + '...');
  ...
  scheduleInteractionCycle() {
    ...
    const runCycle = async () => {
      ...
      console.log('--- Interaction cycle complete ---\n');
  ...
  async registerNewAgent(name, description) {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('\n⚠️  IMPORTANT: Save your API key! It will not be shown again.');
